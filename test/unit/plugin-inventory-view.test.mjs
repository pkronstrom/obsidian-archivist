...
class Element {
 ...
 get text(){return [this.opts.text??"",...this.children.map(c=>c.text)].join(" ");}
...
test("inventory view groups device versions, filters and opens only official store URLs",()=>{
 ...
 const el=new Element(); let filter,refreshed=0;
 api.renderInventoryView(el,{local:r(a,[p("diff","local")],"mobile"),inventories:[r(b,[p("diff","remote"),p("missing"),p("desktop","1",true)]),r(c,[p("missing")])],unavailable:1},"",v=>filter=v,()=>refreshed++);
 ...
 const links=el.all("a"); assert.equal(links.length,2);
 assert.ok(links.every(l=>l.opts.href.startsWith("https://community.obsidian.md/plugins/")));
